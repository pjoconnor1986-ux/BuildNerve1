...
-- Live company calendar. Material deliveries and targeted updates are overlaid from their source tables.
create table if not exists public.calendar_events (
 id uuid primary key default gen_random_uuid(), organisation_id uuid not null references public.organisations(id) on delete cascade,
 project_id uuid references public.projects(id) on delete cascade, title text not null, description text,
 event_type text not null default 'other' check(event_type in ('meeting','delivery','safety','deadline','update','other')),
 starts_at timestamptz not null, ends_at timestamptz, all_day boolean not null default false, location text,
 source_type text not null default 'manual', source_id uuid, created_by uuid not null references public.profiles(id),
 created_at timestamptz not null default now(), updated_at timestamptz not null default now(),
 check(ends_at is null or ends_at>=starts_at)
);
create index if not exists idx_calendar_events_org_start on public.calendar_events(organisation_id,starts_at);
create index if not exists idx_calendar_events_project_start on public.calendar_events(project_id,starts_at);
alter table public.calendar_events enable row level security;
drop policy if exists calendar_events_tenant on public.calendar_events;
create policy calendar_events_tenant on public.calendar_events for all to authenticated
